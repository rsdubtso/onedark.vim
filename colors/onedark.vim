...
call s:h("DiffText", { "fg": s:black, "bg": s:dark_yellow }) " diff mode: Changed text within a changed line
...
call s:h("StatusLine", { "fg": s:white, "bg": s:special_grey }) " status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
call s:h("StatusLineNC", { "fg": s:comment_grey, "bg": s:vertsplit }) " status line of current window
call s:h("StatusLineTerm", { "fg": s:white, "bg": s:special_grey }) " status line of current :terminal window
call s:h("StatusLineTermNC", { "fg": s:comment_grey, "bg": s:vertsplit }) " status line of non-current :terminal window
